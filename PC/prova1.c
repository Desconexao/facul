...
void calcula(int horas_de_frio[5][5], int horas_de_frio_mes_media[5],
             float perc_horas_de_frio_mes[5], float *perc_total_horas_de_frio);
...
int main() {
  ...
  float perc_total_horas_de_frio =
      0.0; // percentual do total de horas em relação à media

  for (m = 0; m < 5; m++)
    for (s = 0; s < 5; s++)
      scanf("%d", &horas_de_frio[m][s]);

  calcula(horas_de_frio, horas_de_frio_mes_media, perc_horas_de_frio_mes,
...
void calcula(int horas_de_frio[5][5], int horas_de_frio_mes_media[5],
  ...
  for (int m = 0; m < 5; m++) {
    ...
    perc_horas_de_frio_mes[m] =
        (float)somaFrioMes / horas_de_frio_mes_media[m] * 100;
  }
